...
class SIAM(nn.Module):
	...
	def forward(self, img, **args):#只需保证两组数据提取出的特征标签顺序一致即可
		h,w = img.shape[-2:]
		if self.flag_down:
			img = F.interpolate(img, size=(h//2,w//2), mode='bilinear', align_corners=False)
		out = self.encoder(img, **args)
		self.pred = self.encoder.pred
		self.feat = self.encoder.feat
		if self.flag_down:
			self.pred = F.interpolate(self.pred, size=(h,w), mode='bilinear', align_corners=False)
			self.feat = F.interpolate(self.feat, size=(h,w), mode='bilinear', align_corners=False)
...
def build_model(type_net='hrdo', type_seg='', type_loss='sim2', type_arch='', num_emb=128):
	...
	if type_seg!='':
		...
		if type_arch=='siam':
			...
			model.__name__ = model.__name__.replace('siamX', '').replace('lunet', 'U')
		elif type_arch=='roma':
