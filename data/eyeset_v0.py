...
class EyeSetResource(object):
	size = dict()
	def __init__(self, folder='../eyeset', dbname='drive', **args):
		...
		self.imgs, self.labs, self.fovs = self.getDataSet(self.dbname)
	...
	def getDataSet(self, dbname):        
		...
		# mats_test = self.readFolder(dbname, part='test', image='mat')
		# 训练集
		...
		# mats_train = self.readFolder(dbname, part='train', image='mat')
		# 全集
		imgs_full,labs_full,fovs_full,mats_full = [],[],[],[]
		...
		# mats_full.extend(mats_train); mats_full.extend(mats_test)
		...
		# db_mats = {'train': mats_train, 'val':mats_train, 'test': mats_test, 'full':mats_full}
		return db_imgs, db_labs, db_fovs#, db_mats
	...
	def readDict(self, index, exeData):  
		...
		return {'img':img, 'lab':lab, 'fov':fov}#
...
TRANS_MAT = OneOf([RandomContrast(p=.7, limit=.3), RandomBrightness(p=.7, limit=.3),])

import itertools
from torch.utils.data import DataLoader, Dataset
class EyeSetGenerator(Dataset, EyeSetResource):
	exeNums = {'train':Aug4CSA.number, 'val':Aug4CSA.number, 'test':1}
	...
	expCross = False   
	LEN_AUG = 6
	def __init__(self, datasize=256, **args):
	...
	def parse(self, pics, cat=True):
		...
		return pics['img'], torch.round(pics['lab']), torch.round(pics['fov']), None

	def post(self, img, lab, index=0):
		return img, lab
	...
	def __getitem__(self, idx, divide=32):
		...
		if self.isTrainMode:
			# 标签平滑
			# pics['lab'] = cv2.GaussianBlur(pics['lab'], ksize=(3,3), sigmaX=0)
			...
			mask = np.stack([pics['lab'], pics['fov'], pics['mat']], axis=-1)
			...
			imag, mask = picaug['image'], picaug['mask']
			pics['img'] = imag
			pics['lab'], pics['fov'], pics['mat'] = mask[:,:,0],mask[:,:,1],mask[:,:,2]
