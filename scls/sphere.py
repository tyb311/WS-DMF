"""
This code is generated by Ridvan Salih KUZU @UNIROMA3
LAST EDITED:  02.03.2020
ABOUT SCRIPT:
It is a script for implementing Additive Angular Margin Penalty, Large Cosine Margin Penalty and Sphere Penalty.
It is based on the papers: https://arxiv.org/pdf/1801.07698.pdf and https://arxiv.org/pdf/1801.09414.pdf
"""

import torch
import torch.nn as nn
import torch.nn.functional as F

#start#
from torch.nn import Parameter
import math

class ArcMarginLoss(nn.Module):
	r"""Implement of large margin arc distance: :
		Args:
			in_features: size of each input sample
			out_features: size of each output sample
			s: norm of input feature
			m: margin
			cos(theta + m)
		"""
	def __init__(self, in_features, out_features, s=32.0, m=0.50, easy_margin=False):
		super(ArcMarginLoss, self).__init__()
		self.in_features = in_features
		self.out_features = out_features
		self.s = s
		self.m = m
		self.weight = Parameter(torch.FloatTensor(out_features, in_features))
		nn.init.xavier_uniform_(self.weight)

		self.easy_margin = easy_margin
		self.cos_m = math.cos(m)
		self.sin_m = math.sin(m)
		self.th = math.cos(math.pi - m)
		self.mm = math.sin(math.pi - m) * m
		# self.lf = torch.nn.LogSoftmax().cuda()
		self.ce = nn.CrossEntropyLoss()

	def forward(self, input, label=None):
		label = label.long()
		# --------------------------- cos(theta) & phi(theta) ---------------------------
		logits = F.linear(F.normalize(input), F.normalize(self.weight))

		sine = torch.sqrt(1.0 - torch.pow(logits, 2))
		phi = logits * self.cos_m - sine * self.sin_m
		if self.easy_margin:
			phi = torch.where(logits > 0, phi, logits)
		else:
			phi = torch.where(logits > self.th, phi, logits - self.mm)
		# --------------------------- convert label to one-hot ---------------------------
		# one_hot = torch.zeros(cosine.size(), requires_grad=True, device='cuda')
		one_hot = torch.zeros(logits.size(), device=input.device)
		one_hot.scatter_(1, label.view(-1, 1), 1)
		# -------------torch.where(out_i = {x_i if condition_i else y_i) -------------
		output = (one_hot * phi) + ((1.0 - one_hot) * logits)  # you can use torch.where if your torch.__version__ is 0.4

		output *= self.s
		# print(output)
		# output=self.lf(output)
		return self.ce(output, label)

class AddMarginLoss(nn.Module):
	r"""Implement of large margin cosine distance: :
	Args:
		in_features: size of each input sample
		out_features: size of each output sample
		s: norm of input feature
		m: margin
		cos(theta) - m
	"""

	def __init__(self, in_features, out_features, s=30.0, m=0.40):
		super(AddMarginLoss, self).__init__()
		self.in_features = in_features
		self.out_features = out_features
		self.s = s
		self.m = m
		self.weight = Parameter(torch.FloatTensor(out_features, in_features))
		nn.init.xavier_uniform_(self.weight)
		self.ce = nn.CrossEntropyLoss()

	def forward(self, input, label):
		label = label.long()
		# --------------------------- cos(theta) & phi(theta) ---------------------------
		logits = F.linear(F.normalize(input), F.normalize(self.weight))
		phi = logits - self.m
		# --------------------------- convert label to one-hot ---------------------------
		one_hot = torch.zeros_like(logits)
		# one_hot = one_hot.cuda() if cosine.is_cuda else one_hot
		one_hot.scatter_(1, label.view(-1, 1), 1)
		# -------------torch.where(out_i = {x_i if condition_i else y_i) -------------
		output = (one_hot * phi) + (
					(1.0 - one_hot) * logits)  # you can use torch.where if your torch.__version__ is 0.4
		output *= self.s
		# print(output)
		return self.ce(output, label)

class SphereLoss(nn.Module):
	r"""Implement of large margin cosine distance: :
	Args:
		in_features: size of each input sample
		out_features: size of each output sample
		m: margin
		cos(m*theta)
	"""
	def __init__(self, in_features, out_features, m=4):
		super(SphereLoss, self).__init__()
		self.in_features = in_features
		self.out_features = out_features
		self.m = m
		self.base = 1000.0
		self.gamma = 0.12
		self.power = 1
		self.LambdaMin = 5.0
		self.iter = 0
		self.weight = Parameter(torch.FloatTensor(out_features, in_features))
		nn.init.xavier_uniform_(self.weight)

		# duplication formula
		self.mlambda = [
			lambda x: x ** 0,
			lambda x: x ** 1,
			lambda x: 2 * x ** 2 - 1,
			lambda x: 4 * x ** 3 - 3 * x,
			lambda x: 8 * x ** 4 - 8 * x ** 2 + 1,
			lambda x: 16 * x ** 5 - 20 * x ** 3 + 5 * x
		]
		self.ce = nn.CrossEntropyLoss()

	def forward(self, input, label):
		label = label.long()
		# lambda = max(lambda_min,base*(1+gamma*iteration)^(-power))
		self.iter += 1
		self.lamb = max(self.LambdaMin, self.base * (1 + self.gamma * self.iter) ** (-1 * self.power))

		# --------------------------- cos(theta) & phi(theta) ---------------------------
		logits = F.linear(F.normalize(input), F.normalize(self.weight))
		logits = logits.clamp(-1, 1)
		cos_m_theta = self.mlambda[self.m](logits)
		theta = logits.data.acos()
		k = (self.m * theta / 3.14159265).floor()
		phi_theta = ((-1.0) ** k) * cos_m_theta - 2 * k
		NormOfFeature = torch.norm(input, 2, 1)

		# --------------------------- convert label to one-hot ---------------------------
		one_hot = torch.zeros(logits.size())
		one_hot = one_hot.cuda() if logits.is_cuda else one_hot
		one_hot.scatter_(1, label.view(-1, 1), 1)

		# --------------------------- Calculate output ---------------------------
		output = (one_hot * (phi_theta - logits) / (1 + self.lamb)) + logits
		output *= NormOfFeature.view(-1, 1)
		return self.ce(output, label)

def get_sphere(mode='arc', num=128):
	if mode == 'arc':
		return ArcMarginLoss(num, num)
	elif mode == 'add':
		return AddMarginLoss(num, num)
	elif mode == 'sph':
		return SphereLoss(num, num)
	else:
		return nn.Sequential()
#end#

def cosine_sim(x1, x2, dim=1, eps=1e-8):
	ip = torch.mm(x1, x2.t())
	w1 = torch.norm(x1, 2, dim)
	w2 = torch.norm(x2, 2, dim)
	return ip / torch.ger(w1, w2).clamp(min=eps)



if __name__ == '__main__':

	feat = torch.rand(99, 256)
	true = torch.rand(99).round()

	los = ArcMarginLoss(256, 256)(feat, true)
	print(los.item())
	los = SphereLoss(256, 256)(feat, true)
	print(los.item())
	los = AddMarginLoss(256, 256)(feat, true)
	print(los.item())