...
class MLPSampler:
	...
	def __init__(self, mode='hard', top=4, low=1, dis=0, num=256, select3=False, roma=False):
		self.top = top
		self.low = low
		self.dis = dis
		self.num = num
	...
	@staticmethod
	def rand(*args):
		return MLPSampler(mode='rand', num=256).select(*args)
	@staticmethod
	def half(*args):
		return MLPSampler(mode='half', num=256).select(*args)
