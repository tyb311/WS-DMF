...
parser.add_argument('--sss', type=str, default='hard', choices=['', 'thsh', 'hard', 'half', 'prob', 'part'], help='Sample Selection Strategy!')
...
if __name__ == '__main__':
	...
	dataset = EyeSetGenerator(dbname=args.db, datasize=args.ds) 
	...
	# keras.test(testset=dataset, key='los', flagSave=True, tta=True)  
	for key in keras.paths.keys():
		keras.test(testset=dataset, key=key, flagSave=True, tta=False)
		# keras.test(testset=dataset, key=key, flagSave=False if key!='a' else True, tta=True)
	...
	keys.append('los')
	for key in keys:
		cmd = f'bash ../datasets/FastSS/script.sh {os.getcwd()}/{keras.root} {key}'
		print(cmd)
		os.system(cmd)
#end#
